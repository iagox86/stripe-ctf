...
class DB(object):
    ...
    def select(self, table, where=None):
        if where is None:
            where = {}
        self.do_select(table, where) #<-- table and where's keys are potentially unsafe
    ...
    def do_select(self, table, where=None):
        where = where or {}
        where_clause = ' AND '.join('%s=?' % key for key in where.iterkeys()) # <-- the 'key' in where isn't paramtarized
        values = where.values()
        q = 'select * from ' + str(table) # <-- table is potentially unsafe
        if where_clause:
