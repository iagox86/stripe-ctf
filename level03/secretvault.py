...
#               blah' UNION select 1, '5fd30c758ce84bce6bb4d33a665911ad0302a332c4500155040ce3d6ffc1e297', 'nacl

userid = 1
password = password
salt = nacl
hash = 5fd30c758ce84bce6bb4d33a665911ad0302a332c4500155040ce3d6ffc1e297
    ...
    user_id, password_hash, salt = res  # <-- we control these 

    calculated_hash = hashlib.sha256(password + salt) # <-- we control the password + salt
    if calculated_hash.hexdigest() != password_hash: # <-- we control this
        return "That's not the password for {0}!\n".format(username)

    flask.session['user_id'] = user_id # <-- we can set this at will
    return flask.redirect(absolute_url('/'))
